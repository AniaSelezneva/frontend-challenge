...
function Like({ id, setLiked }) {
  ...
  const like = () => {
    ...
    if (alreadyLiked) {
      ...
      likedImagesIds = [...alreadyLiked];
      likedImagesIds.push(id); // add new id
    } else {
      likedImagesIds.push(id);
    }
    localStorage.setItem("liked", JSON.stringify(likedImagesIds));
  };

  return (
    <svg
      onClick={like}
      // change color on mouseMove (hover or enter aren't the best options here since
      // the heart remains red when you unlike an image)
      onMouseMove={() => {
        heartSvg.current.setAttribute(
          "d",
