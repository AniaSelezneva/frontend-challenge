...
function Unlike({ id, setLiked }) {
  const unLike = () => {
    ...
    if (alreadyLiked) {
      ...
      // Remove id of unliked image from array
      for (let i = 0; i < alreadyLiked.length; i++) {
        if (alreadyLiked[i] === id) {
          alreadyLiked.splice(i, 1);
          break;
        }
      }
    }

    localStorage.setItem("liked", JSON.stringify(alreadyLiked));
  };
